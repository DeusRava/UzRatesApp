...
import { fetchRates, Rate, getFlagUrl, convert, formatNumber } from '../utils/api';
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                ...
                >
                  {flagUrl ? (
                    <Image source={{ uri: flagUrl }} style={styles.pickerFlag} resizeMode="cover" />
                  ) : (
                    <View style={[styles.pickerFlag, {backgroundColor:'#1E293B',alignItems:'center',justifyContent:'center',borderRadius:4}]}><Text style={{color:'#94A3B8',fontSize:8,fontWeight:'700'}}>{item.code.slice(0,2)}</Text></View>
                  )}
                  <View style={{ flex: 1 }}>
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.block}>
        ...
        <TouchableOpacity style={styles.currencySelector} onPress={() => setShowFrom(true)}>
          {getFlagUrl(fromCode) ? (
            <Image source={{ uri: getFlagUrl(fromCode)! }} style={styles.selectorFlag} resizeMode="cover" />
          ) : (
            <View style={[styles.selectorFlag, {backgroundColor:'#1E293B',alignItems:'center',justifyContent:'center',borderRadius:4}]}><Text style={{color:'#94A3B8',fontSize:10,fontWeight:'700'}}>{fromCode.slice(0,2)}</Text></View>
          )}
          <View style={{ flex: 1 }}>
      ...
      <View style={styles.block}>
        ...
        <TouchableOpacity style={styles.currencySelector} onPress={() => setShowTo(true)}>
          {getFlagUrl(toCode) ? (
            <Image source={{ uri: getFlagUrl(toCode)! }} style={styles.selectorFlag} resizeMode="cover" />
          ) : (
            <View style={[styles.selectorFlag, {backgroundColor:'#1E293B',alignItems:'center',justifyContent:'center',borderRadius:4}]}><Text style={{color:'#94A3B8',fontSize:10,fontWeight:'700'}}>{toCode.slice(0,2)}</Text></View>
          )}
          <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  selectorFlag: { width: 44, height: 30, marginRight: 12, borderRadius: 6 },
  ...
  pickerFlag: { width: 36, height: 24, marginRight: 12, borderRadius: 4 },
